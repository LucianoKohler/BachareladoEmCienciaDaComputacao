...
class Enemy(Element):
	...
	# Funcao atualizada, note que para um algoritmo guloso, existem problemas
	# "gulosos" como o de travar em uma parede, apenas algoritmos mais
	# sofisticados podem sempre chegar ao player 
	def nextNode(self, dirs=["UP", "DOWN", "LEFT", "RIGHT"]):

		melhorNodo = None
		melhorDist = float('inf')

		playerX, playerY = P1.getNodePos()

		# Pra cada ima das quatro direcoes, testa qual é a melhor gulosamente
		# comparando a distancia do inimigo com a do player
		for d in dirs:

			if d == "UP":
				proxX = self.lastNode[0]
				proxY = self.lastNode[1] - 1

				if self.isSafeNode(proxX, proxY):
					dist = abs(proxX - playerX) + abs(proxY - playerY)
					if dist < melhorDist:
						melhorDist = dist
						melhorNodo = (proxX, proxY)
						self.direction = d
				...
				proxX = self.lastNode[0] - 1
				proxY = self.lastNode[1]
