...
-- For
checaComando tg tl retorno (For atrib cond inc bloco) = do
    atrib' <- checaComando tg tl retorno atrib
    cond' <- checaExprLog tg tl cond
    inc' <- checaComando tg tl retorno inc
    bloco' <- mapM(checaComando tg tl retorno) bloco
    return (For atrib' cond' inc' bloco')
