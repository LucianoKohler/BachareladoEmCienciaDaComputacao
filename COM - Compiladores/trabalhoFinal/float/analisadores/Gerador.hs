...
calcularFrame ((id :#: (t, _)):vs) f
    | t == TInt || t == TString || t == TFloat = (id :#: (t, f)) : calcularFrame vs (f + 1)
...
genExpr _ _ _ (Const (CFloat f)) = return (TFloat, geraFloat f)
...
genExpr c tab fun (IntFloat e) = do
    (_, newE) <- genExpr c tab fun e
    return (TFloat, newE ++ "\ti2f\n")

genExpr c tab fun (DoubleInt e) = do
    (_, newE) <- genExpr c tab fun e
    return (TInt, newE ++ "\td2i\n")

genExpr c tab fun (DoubleFloat e) = do
    (_, newE) <- genExpr c tab fun e
    return (TFloat, newE ++ "\td2f\n")

genExpr c tab fun (FloatInt e) = do
    (_, newE) <- genExpr c tab fun e
    return (TInt, newE ++ "\tf2i\n")

genExpr c tab fun (FloatDouble e) = do
    (_, newE) <- genExpr c tab fun e
    return (TDouble, newE ++ "\tf2d\n")
...
genExpr c tab fun (Neg e) = do
    ...
    let cmd = if t == TInt then "\tineg\n" else if t == TFloat then "\tfneg" else "\tdneg\n"
    return (t, newE ++ cmd)
...
iniciaComando (_ :#: (TFloat, f))  = "\tfconst_0\n\tfstore" ++ (if f <= 3 then "_" else " ") ++ show f ++ "\n"
