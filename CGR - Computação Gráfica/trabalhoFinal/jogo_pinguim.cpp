...
bool gameOver = false;
float score = 0.0f;
float bgScrollY = 0.0f; // Variável para controlar o scroll do fundo

// --- DADOS DO JOGADOR (PINGUIM) ---
...
unsigned int loadTexture(const char* path) {
    ...
    if (data) {
        ...
        // Fallback: Cria um pixel preto (bloco) se a imagem não existir
...
void processInput(GLFWwindow* window, float deltaTime) {
    ...
    if (!gameOver) {
        ...
        if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS) {
            gameOver = false;
            score = 0.0f;
            bgScrollY = 0.0f; // Reinicia o scroll ao recomeçar
...
int main() {
    ...
    // Configura o comportamento de repetição vertical (T) especificamente para o fundo
    ...
    // Novas Texturas para o Menu de Game Over
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (!gameOver) {
            ...
            // Atualização do Scroll do Fundo (Proporcional à velocidade dos obstáculos)
            // baseObstacleSpeed está em pixels. Convertemos para espaço UV (0.0 a 1.0) dividindo pela altura da textura (1600.0f)
            bgScrollY += (baseObstacleSpeed / 1600.0f) * deltaTime;
            if (bgScrollY >= 1.0f) bgScrollY -= 1.0f;

            // 1. GERAÇÃO DOS OBSTÁCULOS
        ...
        // --- RENDERIZAÇÃO ORDENADA EM CAMADAS ---
        glClearColor(0.1f, 0.1f, 0.2f, 1.0f);
        glClear(GL_COLOR_BUFFER_BIT);

        // Camada 1: Fundo com Scroll Infinito (Deslocando o retângulo UV verticalmente)
        drawSprite(shaderProgram, VAO, texBackground, glm::vec2(0.0f, 0.0f), glm::vec2(SCREEN_WIDTH, SCREEN_HEIGHT), projection, false, glm::vec4(0.0f, bgScrollY, 1.0f, bgScrollY + 0.5f));

        // Camada 2: Buracos (Ficam no chão, atrás de tudo)
        ...
        // Camada 4: Outros Obstáculos (Morsas e Pinguins, ficam de pé)
        for (const auto& obs : obstacles) {
        ...
        if (!gameOver) {
            ...
            // A. Sprite de "Game Over" (Pulsante)
            ...
            // D. Sprite "Pressione R para reiniciar"
